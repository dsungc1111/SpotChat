...
- 서버에 이미지를 전송할 때, 이미지의 용량이 너무 커서 응답값은 200(성공)으로 받았지만 실제로는 아무 데이터도 전송되지 않아 결과값이 빈 배열([])로 반환
- 한정된 이미지로 테스트했을 때, 문제의 원인을 빠르게 찾지 못했음
...
### 문제상황 - Realm에 데이터 저장 시, 중복 데이터 저장 문제
